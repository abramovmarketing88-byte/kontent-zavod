...
def test_topic_discoverer_unique_run_id(tmp_path: Path) -> None:
    from src.config import InstagramNicheConfig, NicheConfig, Settings
    from src.db import Database
    ...
    brief = (
        ...
        "Сделай Reels на тему: у жизни нет черновика.\n"
    )
    (inbox / "topic.txt").write_text(brief, encoding="utf-8")
    settings = Settings(
        root=tmp_path,
        data_dir=tmp_path / "data",
        jobs_dir=tmp_path / "jobs",
    ...
    assert found[0].title == "У жизни нет черновика"
...
def test_telegram_topic_intake_helpers() -> None:
    ...
    assert parse_topic_text("у жизни нет черновика") == "у жизни нет черновика"
    assert parse_topic_text("/topic ИИ на Авито") == "ИИ на Авито"
    assert parse_topic_text("/start") is None

    brief = format_topic_brief("у жизни нет черновика")
    assert brief.startswith("У жизни нет черновика")
    assert "30–40 секунд" in brief
