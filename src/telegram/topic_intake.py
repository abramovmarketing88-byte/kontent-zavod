...
HELP_TEXT = (
    "🎬 Тема → Reels\n\n"
    "Напиши тему текстом или голосовым — соберу ролик и пришлю в личку.\n\n"
    "Примеры:\n"
...
def _handle_topic(settings: Settings, chat_id: str | int, topic: str) -> bool:
    ...
    if len(topic) < 3:
        _reply(settings, chat_id, "Слишком коротко. Напиши тему подробнее, например: у жизни нет черновика")
        return False
    if len(topic) > 500:
    ...
    _reply(
        settings,
        chat_id,
        f"⏳ Делаю Reels на тему:\n«{title}»\n\nЗаказ #{trigger_id[:8]} — пришлю ролик в личку.",
    )
    return True
